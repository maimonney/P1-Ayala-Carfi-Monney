        ...
        @if ($lastService)
            <div class="col-12 mb-3">
                <div class="card h-100 cont_div">
                    <div class="d-flex flex-column flex-md-row cont_img_vista">
                        @if ($lastService->image && file_exists(public_path($lastService->image)))
                            <img src="{{ asset($lastService->image) }}" class="card-img-top" alt="{{ $lastService->title }}">
                        @elseif ($lastService->image && Storage::disk('public')->exists($lastService->image))
                            <img src="{{ asset('storage/' . $lastService->image) }}" class="card-img-top" alt="{{ $lastService->title }}">
                        @else
                    ...
                    <div class="button-container">
                        <a href="{{ route('servicios.show', $lastService->id) }}" class="button btn_link">Ver más</a>
                    </div>
                </div>
            </div>
        @else
